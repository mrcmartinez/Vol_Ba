...
class PersonalModel extends Model{
    ...
    public function insert($datos){
        try{
            ...
            $query = $conn->prepare('INSERT INTO PERSONAL (ID_PERSONAL,NOMBRE, APELLIDO_PATERNO,
                                                     ...
                                                     FECHA_NACIMIENTO,ESTADO_CIVIL,NUMERO_HIJOS,SEGURO_MEDICO,ESCOLARIDAD,TURNO,HORARIO,ACTIVIDAD,FECHA_INGRESO,ESTATUS) VALUES(:id_personal,:nombre,
                                                      :apellido_paterno,:apellido_materno,:calle,:colonia,
                                                      :numero_exterior,:fecha_nacimiento,:estado_civil,
                                                      :numero_hijos,:seguro_medico,:escolaridad,:turno,:horario,:actividad,:fecha_ingreso,:estatus)');
            $query->execute(['id_personal' => $datos['id_personal'],'nombre' => $datos['nombre'], 'apellido_paterno' => $datos['apellido_paterno'],
                            ...
                            'escolaridad' => $datos['escolaridad'],'turno' => $datos['turno'],'horario' => $datos['horario'],'actividad' => $datos['actividad'],'fecha_ingreso' => $datos['fecha_ingreso'],'estatus' => $datos['estatus']]);
    ...
    public function getBusqueda($c,$f,$h){
     ...
     public function getBusquedaSig($c,$f,$h){
        $items = [];
        try{
            $query = $this->db->connect()->query("SELECT v.*, p.escolaridad,p.numero_hijos,p.fecha_nacimiento,p.estado_civil,p.horario
                                                FROM vistapersonalv as v
                                                INNER JOIN personal as p
                                                ON v.id_personal = p.id_personal
                                                WHERE (v.nombreCompleto like '%".$c."%' OR v.nombreCompletoR like '%".$c."%' OR p.escolaridad like '%".$c."%' OR p.estado_civil like '%".$c."%' OR v.id_personal ='$c' OR DATE_FORMAT(p.fecha_nacimiento,'%M') ='$c') AND p.horario like '%".$h."%' AND v.estatus like '%".$f."%' ORDER BY v.nombreCompleto");
